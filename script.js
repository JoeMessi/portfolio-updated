...
body.addEventListener('click', (e) => {
    ...
    // menu is open and link is clicked
        ...
        setTimeout(() => {window.location = elem.parentNode.href}, 500);
        mainBoxInner.classList.remove('menu-open');
    }
...
body.addEventListener('mouseover', (e) => {
    ...
    // expands border left
    if(elem.classList.contains('click-overlay-hamb')) {
        if(!elem.parentNode.classList.contains('open') && !elem.parentNode.classList.contains('mobile')) {
...
body.addEventListener('mouseout', (e) => {
    var elem = e.target;
     ...
     // srinks border left
    if(elem.classList.contains('click-overlay-hamb') && !elem.parentNode.classList.contains('mobile')) {
        if(!elem.parentNode.classList.contains('open')) {
            borderLeft.classList.remove('expand-leftB');
            elem.parentNode.classList.remove('opening');
        }
...
function drawProject(proj) {
    ...
    document.querySelector('.total-overlay-work .inner .text div.tech').innerHTML = proj.tech.map(t => `<p>${t}</p>`).join('');
    document.querySelector('.total-overlay-work .inner .text .tech-checkout-box .ctas').innerHTML = proj.ctas.map(c => ` <a href="${c.url}" target="_blank"><button>${c.text}</button></a>`).join('');
    document.querySelector('.total-overlay-work .inner .screenshots-box').innerHTML =  proj.images.map(i => `<div class="img-outer"><img src="${i}" alt="${i}"></div>`).join('');
}
